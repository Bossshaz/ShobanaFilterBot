...
_QUALITY_ORDER = ['4K', '2160P', '1080P', '720P', '480P', '360P']
_QUALITY_RE    = re.compile(r'\b(4[kK]|2160[pP]|1080[pP]|720[pP]|480[pP]|360[pP])\b')
_SEASON_RE     = re.compile(r'(?:Season[\s.]*|S)(\d{1,2})(?:\s*E\d+|\b)', re.IGNORECASE)
...
def _extract_quality(name: str) -> str:
    ...
    if not m:
        return 'Other'
    q = m.group(1).upper()
    return '4K' if q in ('4K', '2160P') else q

def _extract_season(name: str):
    m = _SEASON_RE.search(name or '')
...
async def quality_filter_handler(bot, query):
    parts = query.data.split("_", 3)
    if len(parts) != 4:
        return await query.answer("Invalid data.", show_alert=True)
    _, req, key, quality = parts
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("Search for Yourself 🔎", show_alert=True)
    qual_map = QUAL_FILES.get(key)
    if not qual_map:
        return await query.answer("Results expired. Search again.", show_alert=True)
    files = qual_map.get(quality, [])
    if not files:
        return await query.answer("No files for this quality.", show_alert=True)
    settings = await get_settings(query.message.chat.id)
...
async def quality_back_handler(bot, query):
    ...
    if not qual_map:
        return await query.answer("Results expired. Search again.", show_alert=True)
    named_quals = {q: fs for q, fs in qual_map.items() if q != 'Other'}
    btn = []
    for q in sorted(named_quals.keys(), key=lambda x: _QUALITY_ORDER.index(x) if x in _QUALITY_ORDER else 99):
        btn.append([InlineKeyboardButton(
            text=f"🎬 {q}  ({len(named_quals[q])} files)",
            callback_data=f"qual_{req}_{key}_{q}"
        )])
    if 'Other' in qual_map:
        btn.append([InlineKeyboardButton(
            text=f"📁 Other  ({len(qual_map['Other'])} files)",
            callback_data=f"qual_{req}_{key}_Other"
        )])
    total = sum(len(v) for v in qual_map.values())
...
async def season_filter_handler(bot, query):
    parts = query.data.split("_", 3)
    if len(parts) != 4:
        return await query.answer("Invalid data.", show_alert=True)
    _, req, key, snum = parts
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("Search for Yourself 🔎", show_alert=True)
    seas_map = SEASON_FILES.get(key)
    if not seas_map:
        return await query.answer("Results expired. Search again.", show_alert=True)
    files = seas_map.get(int(snum), [])
    if not files:
        return await query.answer("No episodes found.", show_alert=True)
    files = sorted(files, key=lambda f: _extract_episode(f.file_name))
    settings = await get_settings(query.message.chat.id)
    pre = 'filep' if settings['file_secure'] else 'file'
    btn = [[InlineKeyboardButton(
        text=f.file_name,
...
async def season_back_handler(bot, query):
    parts = query.data.split("_", 2)
    if len(parts) != 3:
        return await query.answer()
    _, req, key = parts
    ...
    if not seas_map:
        return await query.answer("Results expired. Search again.", show_alert=True)
    btn = []
    for snum in sorted(seas_map.keys()):
        btn.append([InlineKeyboardButton(
            text=f"📺 Season {snum}  ({len(seas_map[snum])} eps)",
            callback_data=f"seas_{req}_{key}_{snum}"
        )])
    total = sum(len(v) for v in seas_map.values())
...
async def auto_filter(client, msg, spoll=False):
    ...
    if HYPER_MODE:
        ...
        qual_map = {}
        for f in files:
            q = _extract_quality(f.file_name)
            qual_map.setdefault(q, []).append(f)
        named_quals = {q: fs for q, fs in qual_map.items() if q != 'Other'}

        # Season grouping
        seas_map = {}
        for f in files:
            s = _extract_season(f.file_name)
            if s is not None:
                seas_map.setdefault(s, []).append(f)

        use_quality = len(named_quals) >= 2
        use_season  = not use_quality and len(seas_map) >= 2

        if use_quality:
            QUAL_FILES[key] = qual_map
            btn = []
            for q in sorted(named_quals.keys(), key=lambda x: _QUALITY_ORDER.index(x) if x in _QUALITY_ORDER else 99):
                btn.append([InlineKeyboardButton(
                    text=f"🎬 {q}  ({len(named_quals[q])} files)",
            ...
            btn.append([InlineKeyboardButton(text=f"🎞 {len(files)} results — pick quality 👆", callback_data="pages")])

        elif use_season:
            SEASON_FILES[key] = seas_map
            btn = []
            for snum in sorted(seas_map.keys()):
                btn.append([InlineKeyboardButton(
                    text=f"📺 Season {snum}  ({len(seas_map[snum])} eps)",
            ...
            btn.append([InlineKeyboardButton(text=f"🎞 {len(files)} results — pick season 👆", callback_data="pages")])
